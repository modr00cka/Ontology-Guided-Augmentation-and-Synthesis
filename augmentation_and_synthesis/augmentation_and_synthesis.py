    ...
    for x, row in mentions.iterrows():
        if row.ICD9 in labels:
            if random.random()>=1-augmemtation_prob:
                print(f"AUGMENTING LABEL {row.ICD9}")
                ...
                print(str(len(all_replacement_candidates)) + ' candidates')
                ...
                if all_replacement_candidates !=[]:
                    ...
                    print(f"{original_sliced_text} -> {replacement_text}")
...
def augment_all_rows_syn(intext:_df, semehr_output:_df)->_df:
    ...
    print(f'{counter} augmented rows')
...
def synonym_lookup(code:str, synonym_df:_df)->str:
    ...
    """
    return choice((synonym_df[synonym_df["LABEL"]==code].SYNONYMS.iloc[0]).split("|"))
...
def adj_lookup(code:str, conversion_table:_df, codeset:set)->str:
    ...
    alts = conversion_table[conversion_table["code"]==code][codeset].iloc[0].split("|")
    return choice(alts)
...
def augment_row_semehr_adj(row_id:int, text_df:_df, semehr_df:_df, conversion_df:_df, synonym_df:_df, unspecs:list):
    """
    ...
    for x, row in mentions.iterrows():
        ...
        if row.ICD9 in labels and row.ICD9 in unspecs:
            ...
            replacement_candidates.append(synonym_lookup(label_map[row.ICD9], synonym_df))
...
def augment_all_rows_adj(intext:_df, semehr_output:_df, conversion_df:_df, synonym_df:_df)->_df:
	"""
    ...
    for id, row in tqdm(intext.iterrows()):
        new_row = augment_row_semehr_adj(row["ROW_ID"], intext, semehr_output, conversion_df, synonym_df, unspecs)
...
def run_augmentations_adj(orignal_texts_df:_df, traditional_method_results:list, conversion_df:_df, synonym_df:_df, iters =2)->_df:
	"""
	Runs the whole synthesis pipeline over multiple iterations -- as there is randomness involved in choices of codes and of the replacement text for each mention, the same document can yield multiple viable synths. Duplicates are droppped.
	"""
    augmented_texts = []
    for single_method_results in traditional_method_results:
        for _ in range(iters):
            augmented_texts.append(augment_all_rows_adj(orignal_texts_df, medcat_results, conversion_df, synonym_df))
...
if __name__ == "__main__":
    ...
    synonym_path = "/path/to/ViableICD9Synonyms.csv"
    ...
    semehr_results = pd.read_csv("/path/to/semehr/results.csv")
    semehr_results = semehr_results.drop(columns=["Unnamed: 0"]).dropna()
    medcat_results = pd.read_csv(f"/path/to/reformatted/mimic/results.csv")
    ...
    s = 50
    print(f"Initialising augmentation with synonyms with seed {s}")
    seed(s)
    ...
    semehr_synth_texts = run_augmentations_adj(texts, [semehr_results], conv_df, syn_df)
    medcat_synth_texts = run_augmentations_adj(texts, [medcat_results], conv_df, syn_df)
